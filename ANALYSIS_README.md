...
```
python composite_figure.py <input_dir> <output_dir> [--panel-size-in 3.0] [--dpi 300]
```
...
```
python quantify_cells.py <input_dir> <output_dir> [--qc-overlays]
```
...
```
python quantify_cells_shifted.py <input_dir> <output_dir> [--shift-dy 7] [--shift-dx -1]
```
...
- **Replicate subsetting**: filter any `cell_measurements*.csv` on
  `sample.str.extract(r'rep(\d+)')` to restrict to specific replicates
  (e.g. reps 1–2 only — which conveniently also excludes the two known
  merged-cell artifacts from Step 3, since both are in rep 3). Suffix such
  outputs `_rep<N>-<M>`.
